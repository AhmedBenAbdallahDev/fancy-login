...
export const pgChatRepository: ChatRepository = {
  insertThread: async (
    ...
    return {
      ...
      personaId: result.personaId ?? undefined,
      stylePresetId: result.stylePresetId ?? undefined,
      parentThreadId: result.parentThreadId ?? undefined,
      branchedFromMessageId: result.branchedFromMessageId ?? undefined,
    };
  },
  ...
  selectThreadDetails: async (id: string) => {
    ...
    return {
      ...
      characterId: thread.chat_thread.characterId ?? undefined,
      personaId: thread.chat_thread.personaId ?? undefined,
      stylePresetId: thread.chat_thread.stylePresetId ?? undefined,
  ...
  > => {
    const threadWithLatestMessage = await db
      .select({
        ...
        characterId: ChatThreadSchema.characterId,
        personaId: ChatThreadSchema.personaId,
        stylePresetId: ChatThreadSchema.stylePresetId,
        parentThreadId: ChatThreadSchema.parentThreadId,
        branchedFromMessageId: ChatThreadSchema.branchedFromMessageId,
    ...
    return threadWithLatestMessage.map((row) => {
      return {
        ...
        stylePresetId: row.stylePresetId ?? undefined,
        parentThreadId: row.parentThreadId ?? undefined,
        branchedFromMessageId: row.branchedFromMessageId ?? undefined,
  ...
  selectThreadsByCharacterId: async (
    characterId: string,
    userId: string,
  ): Promise<
    (ChatThread & {
    ...
    const threadWithLatestMessage = await db
      ...
      .where(
        and(
          ...
          eq(ChatThreadSchema.characterId, characterId),
        ),
      )
      .groupBy(ChatThreadSchema.id)
      .orderBy(desc(sql`last_message_at`));
