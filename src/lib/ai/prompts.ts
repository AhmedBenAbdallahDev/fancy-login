...
export const buildUserSystemPrompt = (
  ...
  const assistantName = agent?.name || userPreferences?.botName || "Luminar AI";
...
// ============================================================================
// ROLEPLAY / CHARACTER CHAT SYSTEM PROMPTS
// ============================================================================

import { Persona } from "app-types/persona";
import { StylePreset } from "app-types/style-preset";
...
/**
 * Build a system prompt for character/roleplay chat mode.
 * Combines: Writing Style + Character Definition + User's Persona + Roleplay Rules
 */
export const buildRoleplaySystemPrompt = (context: RoleplayContext): string => {
  const { character, persona, style } = context;
  const currentTime = format(new Date(), "EEEE, MMMM d, yyyy 'at' h:mm:ss a");

  let prompt = "";

  // ═══════════════════════════════════════════════════════════════════════════
  // LAYER 1: WRITING STYLE (applies to HOW the AI writes)
  // ═══════════════════════════════════════════════════════════════════════════
...
`;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // LAYER 2: CHARACTER DEFINITION (WHO the AI is)
  // ═══════════════════════════════════════════════════════════════════════════
  prompt += `## CHARACTER: ${character.name}
You are ${character.name}.`;

  if (character.tagline) {
    prompt += ` ${character.tagline}`;
  }
...
${character.personality}`;
  ...
  // Core instructions/system prompt from the character
...
${character.systemPrompt}`;
  ...
  // Example dialogue for AI to learn speaking style
...
${character.exampleDialogue}`;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // LAYER 3: USER'S PERSONA (WHO the user is in this roleplay)
  // ═══════════════════════════════════════════════════════════════════════════
  if (persona) {
    prompt += `

## THE USER: ${persona.name}
The person you're talking to is "${persona.name}".`;
...
Address them as ${persona.name} and interact based on your relationship dynamic.`;
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // LAYER 4: CORE ROLEPLAY RULES
  // ═══════════════════════════════════════════════════════════════════════════
  prompt += `
...
- Express emotions and take actions authentically
- Engage with the conversation immersively
- You can use tools when helpful (like web search) but stay in character when presenting results`;
