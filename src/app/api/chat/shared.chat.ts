...
export function convertToMessage(message: ChatMessage): Message {
  // Extract text content from parts for LLM consumption
  const textContent =
    message.parts
  ...
  return {
    ...message,
    id: message.id,
    content: textContent, // LLM uses this field
    role: message.role,
    parts: message.parts,
 ...
 * Finds the tool in the available tools and executes it
 */
export async function executeSimulatedToolCall(
  toolName: string,
  ...
  try {
    ...
    if (tool.execute) {
      const result = await tool.execute(toolArgs, {
        toolCallId: `simulated_${Date.now()}`,
        messages: [],
    ...
    logger.error(`Error executing simulated tool ${toolName}:`, error);
    return {
      success: false,
      error: errorToString(error),
    };
