...
export const SelectModel = (props: PropsWithChildren<SelectModelProps>) => {
  ...
  const currentDisplayName = useMemo(() => {
    ...
    const match = provider?.models.find((m) => m.name === model.model);
    return match?.displayName || model.model || "model";
  }, [providers, model]);
...
function ProviderIcon({
  ...
  return provider === "anvil" ? (
    <AnvilIcon className={className} />
  ) : provider === "openai" ? (
...
const ProviderHeader = memo(function ProviderHeader({
  ...
  return (
    <div className="text-sm text-muted-foreground flex items-center gap-1.5 group-hover:text-foreground transition-colors duration-300">
      {provider === "anvil" || provider === "openai" ? (
        <ProviderIcon provider={provider} className="size-3 text-foreground" />
      ) : (
