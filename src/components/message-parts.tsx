...
export const UserMessagePart = memo(
  ...
  (prev, next) => {
    ...
    if (prev.isRoleplay != next.isRoleplay) return false;
    if (prev.narrationOpacity != next.narrationOpacity) return false;
...
export const AssistMessagePart = memo(function AssistMessagePart({
  ...
  return (
    <div className={cn(isLoading && "animate-pulse", "flex flex-col gap-2")}>
      ...
      >
        <Markdown
          variant={isRoleplay ? "roleplay" : "default"}
          narrationOpacity={narrationOpacity}
        >
          {part.text}
        </Markdown>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="size-3! p-4! hover:text-primary"
                >
                  {isBranching ? (
                    <Loader className="animate-spin" />
                  ) : (
