...
export default function EditAgent({ id }: { id?: string }) {
  ...
  const saveAgent = useCallback(async () => {
    setIsSaving(true);
    try {
      const validatedAgent = AgentUpsertSchema.parse(agent);
      const body = JSON.stringify(validatedAgent);

      const result = await fetcher(`/api/agent`, {
        method: "POST",
        body,
      });
  ...
  const submitGenerateAgent = useCallback(() => {
    // Reset agent to default state before regenerating
    ...
    <ScrollArea className="h-full w-full relative">
      ...
      <div className="z-10 relative flex flex-col gap-4 px-8 pt-8 pb-14  max-w-3xl h-full mx-auto">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Tagline */}
          <div className="flex flex-col gap-2">
            <Label htmlFor="agent-tagline">{t("Agent.taglineLabel")}</Label>
            {isStoredAgentLoading ? (
              <Skeleton className="w-full h-10" />
            ) : (
              <Input
                id="agent-tagline"
                disabled={isLoading}
                placeholder={t("Agent.taglinePlaceholder")}
                className="hover:bg-input placeholder:text-xs bg-secondary/40 transition-colors border-transparent border-none! focus-visible:bg-input! ring-0!"
                value={agent.tagline || ""}
                onChange={(e) => setAgent({ tagline: e.target.value })}
              />
            )}
          </div>
