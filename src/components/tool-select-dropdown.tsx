...
export function ToolSelectDropdown({
  ...
  const bindingTools = useMemo<string[]>(() => {
    ...
    const defaultTools = Object.values(AppDefaultToolkit)
      .filter((tk) => allowedAppDefaultToolkit?.includes(tk))
      .map((tk) => translate[tk] || tk) // Fallback to key if translation missing
...
function AutoSummaryToggle() {
  const [appStoreMutate, autoSummary] = appStore(
    useShallow((state) => [state.mutate, state.autoSummary]),
  );
  const t = useTranslations();

  const toggleAutoSummary = useCallback(() => {
    appStoreMutate((prev) => ({
      autoSummary: !prev.autoSummary,
    }));
  }, [appStoreMutate]);
    ...
    <DropdownMenuGroup>
      <DropdownMenuItem
        className={cn(
          ...
          autoSummary ? "text-foreground" : "text-muted-foreground",
        )}
        onClick={(e) => {
          e.preventDefault();
          toggleAutoSummary();
      ...
      >
        <svg
          className={cn("size-3.5", autoSummary && "text-foreground")}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        {t("Chat.Tool.autoSummary") || "Auto Summary"}
        <Switch className="ml-auto" checked={autoSummary} />
      </DropdownMenuItem>
    </DropdownMenuGroup>
  );
}
