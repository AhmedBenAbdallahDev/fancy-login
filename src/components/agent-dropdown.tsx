...
export function AgentDropdown({ agent, children, side, align }: Props) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    const previousAgents = appStore.getState().agentList;

    safe()
      .watch(() => setIsDeleting(true))
      // 1. Optimistic Update (Instant Disappearance)
      .ifOk(() => {
        // Update Store
        appStore.setState((prev) => ({
          agentList: prev.agentList.filter((a) => a.id !== agent.id),
        }));

        // Update SWR Cache (so useAgents hook updates immediately)
        mutate(
          "/api/agent",
      ...
      .watch(({ isOk, error }) => {
        if (isOk) {
          ...
          // 3. Final Revalidation (Ensure server sync)
          mutate("/api/agent");
        } else {
          // 4. Rollback on Error
          appStore.setState({ agentList: previousAgents });
          mutate("/api/agent"); // Re-fetch to restore state
