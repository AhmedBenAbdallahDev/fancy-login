...
export type ChatThread = {
  ...
  title: string;
  userId: string;
  // Character/Roleplay mode fields
  characterId?: string; // The character bot being chatted with
  personaId?: string; // User's persona/identity
  stylePresetId?: string; // Writing style preset
  // Branching fields
  parentThreadId?: string; // If this is a branch, the parent thread ID
  branchedFromMessageId?: string; // The message ID where this branch started
  createdAt: Date;
};
...
export type ChatMessage = {
  ...
  isSummary?: boolean;
  summarizedMessageIds?: string[]; // IDs of messages that were summarized
  summarizedAt?: Date;
};
...
export const chatApiSchemaRequestBodySchema = z.object({
  ...
  autoSummary: z.boolean().optional().default(true), // Auto-summarize when context fills
  chatModel: z
    .object({
      provider: z.string(),
      model: z.string(),
